...
%if nargin < 4
 ...
 %   inversion = 'medi'; % can be 'sdi', 'medi', 'star' and 'ndi', other if implemented 
%end

% Default parameters
if nargin < 4 || isempty(bgfr)
    bgfr = 'vsharp'; % 'lbv', 'vsharp', 'pdf'
    inversion = 'medi'; % 'sdi', 'medi', 'star', 'ndi'
end

if nargin < 6 || isempty(thold)
    thold = 0.3; % Default threshold for V-SHARP
end

if nargin < 7 || isempty(lambda)
    lambda = 1500; % Default lambda for MEDI
end
